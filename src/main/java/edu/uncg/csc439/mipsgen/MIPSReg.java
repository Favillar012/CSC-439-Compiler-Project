 ...
 * allocation of objects and a private constructor to make sure there's only
 * one instance of each register object. That way they can be compared just
...
public class MIPSReg {
    ...
    /**
     * The constructor - notice that it is private because register objects
     * should not be created anywhere other than the static initializer
     * above.
     *
    ...
    public static MIPSReg tReg(int num) {
        if (num < 8) return regObj[8+num];
        else if (num < 10) return regObj[16+num];
        else return null;
    }
    ...
    public static MIPSReg sReg(int num) {
        if (num < 8) return regObj[16+num];
        else return null;
    }

    /**
     * Gives an iterable list of temporary registers. Useful for flushing
     * all temporary variables when the end of a basic block is reached.
