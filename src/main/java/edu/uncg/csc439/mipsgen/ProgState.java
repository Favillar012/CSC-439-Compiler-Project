 ...
 * consistently, for each kind of basic operation (copying values, computing
 * new values, etc.)
 *
 * Each IC Address has a "canonical location" where it is considered stored
 * by default (until its location changes through method calls). The first
 * time any name is seen, it is assumed to be stored in its canonical
 * location.
...
public class ProgState {
    ...
    /**
     * Address Descriptor: Tracks the set of registers where each ICAddress is held.
     */
    private final Map<ICAddress, Set<MIPSReg>> addrDesc;

    /**
     * Register Descriptor: Tracks what ICAddress is stored in a register.
     */
    ...
    /**
     * Return the canonical location for an address. Some canonical locations
     * (e.g., for local variables) depend on aspects of the stack frame size,
     * which is set up in the ProgState constructor. Canonical locations for
     * temporary variables are created on the fly, since most temporaries will
     * never need a memory location. To create a new temporary variable location,
     * you call back to the MIPSFunction object for the function that is being
     * compiled, and that can allocate a temporary variable location that is
     * unique within the function. This also allows the function code to know
     * how much space is needed for temporary variables, so that the epilogue
     * and prologue can be created.
     * @param name the name
     * @return the canonical location for the name
     */
    public String getCanonicalLocation(ICAddress name) {
        if (name.isLocal()) {
            ...
            return "-"+(localVarSize+partOf.getTempVariableLocation(name))+"($fp)";
        } else {
    ...
    /**
     * We initialize names/addresses on the demand. This method is called at
     * the top of any method that uses a name, to make sure it exists. Any
     * new name that is seen is initialized to be stored only in its canonical
     * location.
    ...
    /**
     * Gets a current register that has a copy of this name. Usually the first
     * check for where to access a named item, since accessing in a register
     * is fastest. If no register holds a copy of this name, then return null.
    ...
    public MIPSReg getCurrReg(ICAddress name) {
        ...
        else return regs.iterator().next();
    }

    /**
     * Gets an address where a name is stored. In the current implementation,
     * a named data item can only exist at its canonical location, and if that
     * location doesn't hold the current value then null is returned.
     * @param name the name to locate
     * @return its canonical location or null
     */
    public ICAddress getCurrAddress(ICAddress name) {
        initIfNeeded(name);
        if (addrHoldsName.get(name)) return name;
        else return null;
    }
    ...
    /**
     * Update state when a value is copied from a register to a name's
     * canonical location.  Should be called after any kind of "store"
     * instruction is generated. Note that a register should only be
     * copied to a canonical address if the register holds a current copy
     * of the named value. If this isn't the case, a RuntimeException is
     * thrown.
     *
     * @param name destination name
     * @param reg source register
     */
    public void storeValue(ICAddress name, MIPSReg reg) {
    ...
    /**
     * Update state when a value is copied from an address to a register, where
     * the address is given by the location's canonical name. Should be called
     * after any kind of "load" instruction is generated. Note that if you try
     * to copy from a name's canonical address, and that address doesn't hold
     * a current copy of the name, then a RuntimeException is thrown. This
     * should never happen!
     *
     * @param reg destination register
     * @param name source - use null if no source address (i.e., constant)
     */
    public void loadValue(MIPSReg reg, ICAddress name) {
        initIfNeeded(name);
    ...
    /**
     * Update state when a value is copied from one register to another.
     * Reflects a MIPS "move" instruction
     * @param dreg destination register
     * @param sreg source register
     */
    public void moveValue(MIPSReg dreg, MIPSReg sreg) {
    ...
    /**
     * Set this register as the unique location for a new (replacement) value
     * for the name. Invalidates all other storage locations for that name since
     * they refer to the previous value. Used after a computation instruction
     * creates a new value for the name.
     *
     * @param reg  the new, unique, register location for the name
     * @param name the name
     */
    public void newValForName(MIPSReg reg, ICAddress name) {
        initIfNeeded(name);
        // Register loses it's prior contents and names lose that storage loc
        for (ICAddress a : regDesc.get(reg)) {
            addrDesc.get(a).remove(reg);
     ...
     * @param name the name
     */
    public void setNameIn(MIPSReg reg, ICAddress name) {
        initIfNeeded(name);
        if (!regDesc.get(reg).contains(name)) {
    ...
    /**
     * Invalidate a register - removes it as a storage location for all the
     * names currently there. Doesn't make sure there's another copy elsewhere,
     * so be careful not to lose your values!
     *
    ...
    /**
     * Invalidate a name - removes any references to that name in any register,
     * which should be done when you are finished with the name (so registers
     * aren't held unnecessarily). It is up to the caller to make sure the value
     * is saved first, if needed later (like in another block).
     */
    public void freeName(ICAddress addr) {
        Set<MIPSReg> regSet = addrDesc.get(addr);
        if (regSet != null) {
            for (MIPSReg reg : regSet) {
                regDesc.get(reg).remove(addr);
            }
            regSet.clear();
        }
    }
     ...
     * changing reg so that you can store any of these values back to the
     * canonical address.
     *
    ...
    /**
     * Get an available register, avoiding two specific registers (may be
     * null if no restrictions). Get a free register if possible, and if not
     * just pick the first non-avoiding register.
     * @param avoid1 don't use this register
     * @param avoid2 or this register
     * @return a register that can be used
     */
    public MIPSReg getAvailableRegister(MIPSReg avoid1, MIPSReg avoid2) {
